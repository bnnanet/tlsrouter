...
func (lc *ListenConfig) matchService(conf *Config, domain string, alpns []string) (SNIALPN, *ConfigService, error) {
	...
	// Key includes ALPNs: resolution is ALPN-dependent (CNAME/SRV), so a shared
	// flight across different ALPN sets would return the wrong backend to the
	// second caller.
	resolveKey := domain + ":" + strings.Join(alpns, ",")
	resultAny, resolveErr, _ := lc.resolveGroup.Do(resolveKey, func() (any, error) {
